local Translations = {
	warning = {
		...
		wrong_branch_warning = '~b~spark~w~: ~o~Warnung~w~: Dies ist der Entwicklungs-Branch (master)',
		wrong_branch_info = '~y~DIESE VERSION IST IN ENTWICKLUNG UND WIRD NICHT\nFÜR DEN PRODUKTIONSGEBRAUCH EMPFOHLEN. WENN DIES EIN FEHLER WAR, DANN DOWNLOAD DIE\nNEUESTEN STABLE-RELEASE UNTER:\n~g~github.com/AgentBUB/spark~p~~h~/releases~h~',
		wrong_branch_mute = '~b~ZUM STUMMSCHALTEN~w~: Setze die CONVAR "~o~experimental~w~" auf "~o~true~w~" in der fxmanifest.',
		profile_missing = '^3spark(%{ver}) WARNUNG: "DEFAULT" Tabelle fehlt aus der %{tbl} Tabelle. Es wird eine leere Tabelle für %{model} verwendet.',
		-- SIREN CONTROLLER --
		too_few_tone_frontend = '~b~spark ~y~Warnung %{code}:~s~ Zu wenig Sirenen zugewiesen.',
		too_few_tone_console = '^3spark(%{ver}) Warnung %{code}: Zu wenig Sirenen zugewiesen. Mindestens %{req_tone_count} Töne erforderlich. (UTIL:SetToneByPos(%{tone_string}, %{pos})',
		tone_position_nil_frontend = '~b~spark ~y~Warnung %{code}:~s~ Es wurde versucht einen Ton einzustellen, wobei kein approved_tones ausfindig gemacht werden konnte. Siehe Konsole.',
		tone_position_nil_console = '^3spark(%{ver}) Warnung %{code}: Es wurde versucht den Ton "%{tone_string}" einzustellen, aber es konnte die Tabelle approved_tones nicht ausfindig gemacht werden. (UTIL:SetToneByPos(%{tone_string}, %{pos}). Versuchen Sie, die Einstelluingen auf die Werkseinstellungen zurückzusetzen, da dies nach einer Änderung der Sirenentonzuordnung auftreten kann.',
		tone_id_nil_frontend = '~b~spark ~y~Warnung %{code}:~s~ Es wurde versucht einen Ton einzustellen, welcher nicht in approved_tones gefunden werden konnte. Siehe Konsole.',
		tone_id_nil_console = '^3spark(%{ver}) Warnung %{code}: Es wurde versucht den Ton %{tone_string} einzustellen, aber es konnte die Pos %{tone_id} in der approved_tones nicht gefunden werden. (UTIL:SetToneByPos(%{tone_string}, %{pos}). Versuchen Sie, die Einstelluingen auf die Werkseinstellungen zurückzusetzen, da dies nach einer Änderung der Sirenentonzuordnung auftreten kann.',
		factory_reset = 'Sind Sie sicher, dass Sie alle gespeicherten spark-Daten löschen und auf die Werkseinstellungen zurücksetzen möchten?',
		facory_reset_options= '~g~Nein: Escape \t ~r~Ja: Enter',
	},
	error = {
		-- CONFIG ERRORS -- 
		missing_community_id_frontend = '~b~~h~spark~h~ ~r~~h~KONFIGURATIONSFEHLER~h~~s~: COMMUNITY-ID FEHLT. SIEHE LOGS. SERVER-ENTWICKLER KONTAKTIEREN.',
		missing_community_id_console = '^KONFIGURATIONSFEHLER: COMMUNITY ID NICHT GESETZT, DIES IST ERFORDERLICH, UM KONFLIKTE FÜR SPIELER ZU VERMEIDEN, DIE AUF MEHREREN SERVERN MIT spark SPIELEN. BITTE SETZEN SIE DIES IN DER SETTINGS.LUA.',
		invalid_resource_name_frontend = '~b~~h~spark~h~ ~r~~h~KONFIGURATIONSFEHLER~h~~s~: UNGÜLTIGER RESSOURCENNAME. SIEHE LOGS. SERVER-ENTWICKLER KONTAKTIEREN.',
		invalid_resource_name_console = '^KONFIGURATIONSFEHLER: UNGÜLTIGER RESSOURCENNAME. BITTE ÜBERPRÜFEN SIE, OB DER NAME DES RESSOURCENORDNERS "^3spark^1" (CASE-SENSITIVE) LAUTET. DIES IST FÜR DIE ORDNUNGSGEMÄSSE SPEICHER-/LADEFUNKTION ERFORDERLICH. BITTE UMBENENNEN, AKTUALISIEREN UND SICHERSTELLEN.',
		resource_conflict_frontend = '~b~~h~spark~h~ ~r~~h~KONFLIKTFEHLER~h~~s~: RESSOURCENKONFLIKT. SIEHE KONSOLE.',
		resource_conflict_console = '^1spark ERROR: "lux_vehcontrol" LÄUFT BEREITS UND STEHT IM KONFLIKT MIT spark. BITTE STOPPEN SIE "lux_vehcontrol" UND STARTEN SIE spark NEU.',
		-- FRONTEND ERRORS --
		reg_keymap_nil_1 = '~b~~h~spark~h~ ~r~~h~ERROR 2~h~~s~: Nullwert gefangen.\nDetails: (%{i}, %{proposed_tone}, %{profile_name})',
		reg_keymap_nil_2 = '~b~~h~spark~h~ ~r~~h~ERROR 2~h~~s~: Versuchen Sie, das Fahrzeug zu wechseln und wieder zurückzuschalten ODER die Profileinstellungen zu laden (falls vorhanden).',
		profile_nil_table_frontend = '~b~~h~spark~h~ ~r~ERROR: %{tbl} hat versucht, das Profil aus der Tabelle "nil" abzurufen. Siehe Konsole.',
		profile_nil_table_console = '^1spark(%{ver}) ERROR: %{tbl} versucht, das Profil aus der Tabelle "nil" zu holen. Dies wird normalerweise durch ein ungültiges Zeichen oder eine fehlende { }-Klammer in der SIRENS.lua verursacht. (https://git.io/JDVhK)',
	},
	info = {
		...
		debug_mode_console = '^3spark Info: Debug-Modus vorübergehend auf %{state} gesetzt. Debug_mode wird nach einem Neustart der Ressourcen zurückgesetzt, sofern nicht in der fxmanifest festgelegt. Stellen Sie sicher, dass Sie "refresh" ausführen, um die Änderungen von fxmanifest zu sehen.',
		profile_found = '^4spark(%{ver}) ^5%{tbl}: ^7Profil %{profile} gefunden für %{model}.',
		profile_default_console = '^4spark(%{ver}) ^5%{tbl}: ^7Verwendung des Standardprofils für %{model}.',
		profile_default_frontend = '~b~spark~s~: Verwendung des ~b~STANDARD~s~ Profil für \'~o~ %{model} ~s~\'.',
		extra_on = '^4spark: ^7%{extra} einschalten',
		extra_off = '^4spark: ^7%{extra} ausschalten',
		unknown = 'unbekannt',
		...
		factory_reset_success_frontend = '~g~Erfolg~s~: Sie haben alle gespeicherten Daten gelöscht und spark zurückgesetzt.',
	},
	control = {
		siren_control_desc = 'spark Sirene: %{ord_num}',
		lock_desc = 'spark: Sperren der Kontrollen',
		menu_desc = 'spark: Menü öffnen',
	},
	command = {
		lock_command = 'sparklock',
		lock_desc = 'Umschalten der Luxart Vehicle Kontroll-Tastenbelegung.',
		debug_command = 'sparkdebug',
	...
	menu = {
		...
		save_desc = 'spark-Einstellungen speichern.',
		...
		load_desc =  'spark-Einstellungen laden.',
		...
		reset_desc = '~r~Setzt spark auf den Standardzustand zurück, wobei vorhandene Speicherungen erhalten bleiben. Setzt ungespeicherte Einstellungen außer Kraft.',
		reset_success = '~g~Erfolg~s~: Die Einstellungen wurden zurückgesetzt.',
		factory_reset = 'Werkseinstellung',
		factory_reset_desc = '~r~Dauerhaftes Löschen von Speicherständen und Zurücksetzen von spark in den Standardzustand.',
		load_copy = 'Laden',
	...
	plugins ={
		...
		ec_not_approved_console = '^3spark Warnung P404: versuch das Steuerelement %{control} zu verwenden, konnte aber CONTROLS nicht finden. %{type} Tabelle. Versuchen Sie auf die Werkseinstellungen zurückzusetzen oder melden Sie sich beim Serverentwickler.',
		ec_not_approved_frontend = '~b~spark ~y~Warnung P404:~s~ versuch die Kontrolle zu nutzen, die aber nicht zugelassen wurde. Siehe Konsole.',
		ec_fail_load_console = '^3spark Warnung:  Die gespeicherte Kontrolle für \'${name}\' ist vom Serverentwickler nicht mehr erlaubt. Rückkehr zum Standard. Speichern Sie das Steuerprofil erneut, um diesen Fehler zu beheben. STEUERUNG: %{control}',
		ec_fail_load_frontend = '~b~spark ~y~Warnung: Steuerung kann nicht geladen werden für \'%{name}\'. Siehe Konsole.',
		ec_save_not_used = '^3spark Info: Es wurden Speicherdaten gefunden, die nicht mit der aktuellen Konfiguration von Extra Controls übereinstimmen. Wahrscheinlich handelt es sich um alte Daten, die inzwischen von einem Serverentwickler geändert wurden. Sie können diese durch erneutes Speichern löschen.',
		-------------------
		...
		ei_control_desc = 'spark Verdunklung umschalten',
		ei_command_desc = 'spark-Verdunklungs-Modus umschalten.',
		ei_invalid_extra = '^3spark Info: EXTRA_INTEGRATION Tabelle enthält nicht existierendes Extra: %{extra} für %{profile}.',
		-------------------
		...
		ta_control_desc_left = 'spark Umschalten Links TA',
		ta_control_desc_right = 'spark Umschalten Rechts TA',
		ta_control_desc_middle = 'spark Umschalten Mitte TA',
		-------------------
