local Translations = {
	warning = {
		...
		wrong_branch_warning = '~b~spark~w~: ~o~Warning~w~: This is the development branch (master)',
		...
		profile_missing = '^3spark(%{ver}) WARNING: "DEFAULT" table missing from %{tbl} table. Using empty table for %{model}.',
		-- SIREN CONTROLLER --
		too_few_tone_frontend = '~b~spark ~y~Warning %{code}:~s~ too little sirens assigned.',
		too_few_tone_console = '^3spark(%{ver}) Warning %{code}: too little sirens assigned. Minimum %{req_tone_count} tones required. (UTIL:SetToneByPos(%{tone_string}, %{pos})',
		tone_position_nil_frontend = '~b~spark ~y~Warning %{code}:~s~ attempted to set tone but, was unable to locate approved_tones. See console.',
		tone_position_nil_console = '^3spark(%{ver}) Warning %{code}: attempted to set tone "%{tone_string}" but, was unable to locate approved_tones table. (UTIL:SetToneByPos(%{tone_string}, %{pos}). Try factory resetting as this may occur after siren tone assignments change.',
		tone_id_nil_frontend = '~b~spark ~y~Warning %{code}:~s~ attempted to set tone but, was unable to locate in approved_tones. See console.',
		tone_id_nil_console = '^3spark(%{ver}) Warning %{code}: attempted to set tone %{tone_string} but, was unable to locate position: %{tone_id} in approved_tones. (UTIL:SetToneByPos(%{tone_string}, %{pos}). Try factory resetting as this may occur after siren tone assignments change.',
		factory_reset = 'Are you sure you want to delete all saved spark data and Factory Reset?',
		facory_reset_options= '~g~No: Escape \t ~r~Yes: Enter',
	},
	error = {
		-- CONFIG ERRORS -- 
		missing_community_id_frontend = '~b~~h~spark~h~ ~r~~h~CONFIG ERROR~h~~s~: COMMUNITY ID MISSING. SEE LOGS. CONTACT SERVER DEVELOPER.',
		missing_community_id_console = '^1CONFIG ERROR: COMMUNITY ID NOT SET, THIS IS REQUIRED TO PREVENT CONFLICTS FOR PLAYERS WHO PLAY ON MULTIPLE SERVERS WITH spark. PLEASE SET THIS IN SETTINGS.LUA.',
		invalid_resource_name_frontend = '~b~~h~spark~h~ ~r~~h~CONFIG ERROR~h~~s~: INVALID RESOURCE NAME. SEE LOGS. CONTACT SERVER DEVELOPER.',
		invalid_resource_name_console = '^1CONFIG ERROR: INVALID RESOURCE NAME. PLEASE VERIFY RESOURCE FOLDER NAME READS "^3spark^1" (CASE-SENSITIVE). THIS IS REQUIRED FOR PROPER SAVE / LOAD FUNCTIONALITY. PLEASE RENAME, REFRESH, AND ENSURE.',
		resource_conflict_frontend = '~b~~h~spark~h~ ~r~~h~CONFLICT ERROR~h~~s~: RESOURCE CONFLICT. SEE CONSOLE.',
		resource_conflict_console = '^1spark ERROR: DETECTED "lux_vehcontrol" RUNNING, THIS CONFLICTS WITH spark. PLEASE STOP "lux_vehcontrol" AND RESTART spark.',
		profile_none_found_frontend = '~b~~h~spark~h~ ~r~~h~CONFIG ERROR~h~~s~: DEFAULT TABLE MISSING. SEE LOGS. CONTACT SERVER DEVELOPER.',
		profile_none_found_console = '^1CONFIG ERROR: UNABLE TO FIND A PROFILE FOR \'^3%{game_name}^1\', AND REQUIRED FALLBACK TABLE \'DEFAULT\' IS NOT PRESENT. (https://bit.ly/spark-CSATS)',

		-- FRONTEND ERRORS --
		reg_keymap_nil_1 = '~b~~h~spark~h~ ~r~~h~ERROR 2~h~~s~: Nil value caught.\ndetails: (%{i}, %{proposed_tone}, %{profile_name})',
		reg_keymap_nil_2 = '~b~~h~spark~h~ ~r~~h~ERROR 2~h~~s~: Try switching vehicles and switching back OR loading profile settings (if save present).',
		profile_nil_table_frontend = '~b~~h~spark~h~ ~r~ERROR: %{tbl} attempted to get profile from nil table. See console.',
		profile_nil_table_console = '^1spark(%{ver}) ERROR: %{tbl} attempted to get profile from nil table. This is typically caused by an invalid character or missing { } brace in SIRENS.lua. (https://git.io/JDVhK)',
	},
	info = {
		...
		debug_mode_console = '^3spark Info: debug mode set to %{state} temporarily. Debug_mode resets after resource restart unless set in fxmanifest. Make sure to run "refresh" to see fxmanifest changes.',
		profile_found = '^4spark(%{ver}) ^5%{tbl}: ^7profile %{profile} found for %{model}.',
		disabled_profile = '^4spark(%{ver}) ^5%{tbl}: ^1spark disabled for %{model} using profile %{profile}.',
		profile_default_console = '^4spark(%{ver}) ^5%{tbl}: ^7using default profile for %{model}.',
		profile_default_frontend = '~b~spark~s~: Using ~b~DEFAULT~s~ profile for \'~o~ %{model} ~s~\'.',
		extra_on = '^4spark: ^7Toggling %{extra} on',
		extra_off = '^4spark: ^7Toggling %{extra} off',
		unknown = 'unknown',
		...
		factory_reset_success_frontend = '~g~Success~s~: You have deleted all save data and reset spark.',
	},
	control = {
		siren_control_desc = 'spark Siren: %{ord_num}',
		lock_desc = 'spark: Lock out controls',
		menu_desc = 'spark: Open Menu',
	},
	command = {
	...
	menu = {
		...
		hud_backlight_on = 'On',
		hud_backlight_desc = 'Changes HUD backlight behavior. ~b~Auto~s~ is determined by headlight state. | Only works on default controller.',
		hud_reset = 'Reset',
		...
		reset_desc = '~r~Reset spark to it\'s default state, preserves existing saves. Will override any unsaved settings.',
		reset_success = '~g~Success~s~: Settings have been reset.',
		factory_reset = 'Factory Reset',
		factory_reset_desc = '~r~Permanently delete any saves, resetting spark to its default state.',
		load_copy = 'Load',
	...
	plugins ={
		...
		ec_controller_support_desc = 'When disabled, spark ignores controller button input. Requires combo-key to be None',
		ec_not_approved_console = '^3spark Warning P404: attempted to use control %{control} but, was unable to locate CONTROLS.%{type} table. Try factory resetting or report to server developer.',
		ec_not_approved_frontend = '~b~spark ~y~Warning P404:~s~ attempted to use control but was not approved. See console.',
		ec_fail_load_console = '^3spark Warning:  The saved control for \'${name}\' is no longer permitted by server developer. Reverting to default. Re-save control profile to remove this error. CONTROL: %{control}',
		ec_fail_load_frontend = '~b~spark ~y~Warning: Unable to load control for \'%{name}\'. See console.',
		ec_save_not_used = '^3spark Info: found save data that did not align with current Extra Controls configuration. Likely old data that has since been changed by a server developer. You can delete this by re-saving.',
		-------------------
		...
		ei_command_desc = 'Toggle spark Blackout Mode.',
		ei_invalid_extra = '^3spark Info: EXTRA_INTEGRATION table contains non-existent extra: %{extra} for %{profile}.',
		-------------------
		...
		ta_control_desc_left = 'spark Toggle Left TA',
		ta_control_desc_right = 'spark Toggle Right TA',
		ta_control_desc_middle = 'spark Toggle Middle TA',
		-------------------
