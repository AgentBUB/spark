...
local save_prefix = 'spark_'..community_id..'_'
...
--	This should never be removed. It is the only easy way for end users to delete spark data.
RegisterCommand('sparkfactoryreset', function(source, args)
...
--if GetResourceMetadata(GetCurrentResourceName(), 'debug_mode', 0) == 'true' then
	RegisterCommand('sparkdumpkvp', function(source, args)
		UTIL:Print('^4spark ^5STORAGE: ^7Dumping KVPs...')
		local handle = StartFindKvp(save_prefix);
		local key = FindKvp(handle)
		while key ~= nil do
			if GetResourceKvpString(key) ~= nil then
				UTIL:Print('^4spark ^5STORAGE Found: ^7"'..key..'" "'..GetResourceKvpString(key)..'", STRING', true)
			elseif GetResourceKvpInt(key) ~= nil then
				UTIL:Print('^4spark ^5STORAGE Found: ^7"'..key..'" "'..GetResourceKvpInt(key)..'", INT', true)
			elseif GetResourceKvpFloat(key) ~= nil then
				UTIL:Print('^4spark ^5STORAGE Found: ^7"'..key..'" "'..GetResourceKvpFloat(key)..'", FLOAT', true)
			end
			key = FindKvp(handle)
			Wait(0)
		end
		UTIL:Print('^4spark ^5STORAGE: ^7Finished Dumping KVPs...')
	end)
--end
------------------------------------------------
-- Resource Start Initialization
CreateThread(function()
	TriggerServerEvent('spark:GetRepoVersion_s')
...
function STORAGE:DeleteKVPs(prefix)
	...
	while key ~= nil do
		...
		UTIL:Print('^3spark Info: Deleting Key \'' .. key .. '\'', true)
		key = FindKvp(handle)
		Wait(0)
	end
end
...
function STORAGE:SaveHUDSettings()
	...
	SetResourceKvp(save_prefix .. 'hud_data_'.. controller,  json.encode(hud_save_data))
end


--[[Saves all KVP values.]]
function STORAGE:SaveSettings()
	UTIL:Print('^4spark: ^5STORAGE: ^7Saving Settings...')
	...
	if custom_tone_names then
		...
		UTIL:Print('^4spark ^5STORAGE: ^7saving '..save_prefix..'tone_names...')		
	end
	...
	if UTIL:GetVehicleProfileName() ~= nil then
		...
		if profile_name ~= nil then
			...
			UTIL:Print('^4spark ^5STORAGE: ^7saving '..save_prefix .. 'profile_'..profile_name..'!')
			...
			UTIL:Print('^4spark ^5STORAGE: ^7saving profile_'..profile_name..'_audio_data')
		else
			HUD:ShowNotification('~b~spark: ~r~SAVE ERROR~s~: profile_name after gsub is nil.', true)
		end
	else
		HUD:ShowNotification('~b~spark: ~r~SAVE ERROR~s~: UTIL:GetVehicleProfileName() returned nil.', true)
	end
	UTIL:Print('^4spark ^5STORAGE: ^7Finished Saving Settings...')
end

------------------------------------------------
--[[Loads all KVP values.]]
function STORAGE:LoadSettings(profile_name)	
	UTIL:Print('^4spark ^5STORAGE: ^7Loading Settings...')
	...
	if incompatible then
		AddTextEntry('spark_mismatch_version','~y~~h~Warning:~h~ ~s~Luxart Vehicle Control Save Version Mismatch.\n~b~Compatible Version: ' .. comp_version .. '\n~o~Save Version: ' .. save_version .. '~s~\nYou may experience issues, to prevent this message from appearing verify settings and resave.')
		SetNotificationTextEntry('spark_mismatch_version')
		DrawNotification(false, true)
	end
	
	local hud_save_data = GetResourceKvpString(save_prefix..'hud_data_'..controller)
	if hud_save_data ~= nil then
		...
		UTIL:Print('^4spark ^5STORAGE: ^7loaded HUD data.')		
	end
	...
	if save_version ~= nil then
		...
		if main_siren_settings_masterswitch then
			...
			UTIL:Print('^4spark ^5STORAGE: ^7loaded custom tone names.')
		end
		...
		if UTIL:GetVehicleProfileName() ~= false then
			...
			if profile_name ~= nil then
				...
				if profile_save_data ~= nil then
					...
					UTIL:Print('^4spark ^5STORAGE: ^7loaded '..profile_name..'.')
				end
				--Audio Settings 
				...
				if audio_save_data ~= nil then
					...
					UTIL:Print('^4spark ^5STORAGE: ^7loaded audio data.')
				end
			else
				HUD:ShowNotification('~b~spark:~r~ LOADING ERROR~s~: profile_name after gsub is nil.', true)
			end
		end
	end
	UTIL:Print('^4spark ^5STORAGE: ^7Finished Loading Settings...')
end

------------------------------------------------
--[[Resets all KVP/menu values to their default.]]
function STORAGE:ResetSettings()
	UTIL:Print('^4spark ^5STORAGE: ^7Resetting Settings...')
	...
	UTIL:Print('^4spark ^5STORAGE: ^7Finished Resetting Settings...')
end
...
RegisterNetEvent('spark:SendRepoVersion_c')
AddEventHandler('spark:SendRepoVersion_c', function(version)
