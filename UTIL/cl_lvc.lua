...
---------------------------------------------------
FILE: cl_lvc.lua
PURPOSE: Core Functionality and User Input for LVC
---------------------------------------------------
...
CreateThread(function()
	if GetResourceState('lvc') ~= 'started' and GetResourceState('lvc') ~= 'starting' or GetResourceState('redneck-lighting') ~= 'started' and GetResourceState('lvc') ~= 'redneck-lighting' then
			...
			if community_id ~= nil and community_id ~= '' then
				while true do
					...
					if IsPedInAnyVehicle(playerped, false) then
						...
						controller = UTIL:LoadVehicleConfig(veh)?.controller or 'default'
		...
		if player_is_emerg_driver then
			while playerped ~= nil and veh ~= nil do
				if GetIsTaskActive(playerped, 2) and GetVehiclePedIsIn(ped, true) then
					TriggerEvent('spark:onVehicleExit')
					Wait(1000)
				end
				Wait(0)
			end
	...
	while true do	
		if player_is_emerg_driver and veh ~= nil then
			if last_veh == nil then
				TriggerEvent('spark:onVehicleChange')
			else
				if last_veh ~= veh then
					if HUD:GetHudState() then
						HUD:SetItemState('hud', false, last_controller)
						HUD:SetItemState('hud', true, controller)
					end
					TriggerEvent('spark:onVehicleChange')
				end
			end
...
--Kill siren on Exit
RegisterNetEvent('spark:onVehicleExit')
AddEventHandler('spark:onVehicleExit', function()
...
RegisterKeyMaps = function()
	for i, _ in ipairs(SIRENS) do
		if i ~= 1 then
			local command = '_spark_siren_' .. i-1
...
BroadcastPlayerVehicleState = function(vehicle)
	if veh == vehicle then
		...
		TriggerEvent('spark:UpdateThirdParty', update_data)
	end
end	
...
----------------EVENT HANDLERS------------------
------------------------------------------------
RegisterNetEvent('spark:TogIndicState_c')
AddEventHandler('spark:TogIndicState_c', function(sender, newstate)
...
---------------------------------------------------------------------
RegisterNetEvent('spark:TogDfltSrnMuted_c')
AddEventHandler('spark:TogDfltSrnMuted_c', function(sender)
	local player_s = GetPlayerFromServerId(sender)
	local ped_s = GetPlayerPed(player_s)
	if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
		if ped_s ~= GetPlayerPed(-1) then
...
---------------------------------------------------------------------
RegisterNetEvent('spark:SetLxSirenState_c')
AddEventHandler('spark:SetLxSirenState_c', function(sender, newstate)
	local player_s = GetPlayerFromServerId(sender)
	local ped_s = GetPlayerPed(player_s)
	if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
		if ped_s ~= GetPlayerPed(-1) then
...
---------------------------------------------------------------------
RegisterNetEvent('spark:SetPwrcallState_c')
AddEventHandler('spark:SetPwrcallState_c', function(sender, newstate)
	local player_s = GetPlayerFromServerId(sender)
	local ped_s = GetPlayerPed(player_s)
	if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
		if ped_s ~= GetPlayerPed(-1) then
...
---------------------------------------------------------------------
RegisterNetEvent('spark:SetAirManuState_c')
AddEventHandler('spark:SetAirManuState_c', function(sender, newstate)
	local player_s = GetPlayerFromServerId(sender)
	local ped_s = GetPlayerPed(player_s)
	if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
		if ped_s ~= GetPlayerPed(-1) then
		...
		if GetPedInVehicleSeat(veh, -1) == playerped then
					...
					if not IsPauseMenuActive() and UpdateOnscreenKeyboard() ~= 0 and not radio_wheel_active then
						if not key_lock then
							...
							if IsDisabledControlJustReleased(0, 85) then
								if lights_on and UTIL:LoadVehicleConfig(veh)?.stages == nil then
								...
								elseif lights_on and UTIL:LoadVehicleConfig(veh)?.stages ~= nil then
									AUDIO:Play('On', AUDIO.on_volume)
									if IsVehicleExtraTurnedOn(veh, UTIL:LoadVehicleConfig(veh)?.stages.one[1]) and not IsVehicleExtraTurnedOn(veh, UTIL:LoadVehicleConfig(veh)?.stages.two[1]) then
										for _,k in pairs(UTIL:LoadVehicleConfig(veh)?.stages.two) do
											SetVehicleExtra(veh, k, 0)
										end
									elseif IsVehicleExtraTurnedOn(veh, UTIL:LoadVehicleConfig(veh)?.stages.two[1]) and not IsVehicleExtraTurnedOn(veh, UTIL:LoadVehicleConfig(veh)?.stages.three[1]) then
										for _,k in pairs(UTIL:LoadVehicleConfig(veh)?.stages.three) do
											SetVehicleExtra(veh, k, 0)
										end
									elseif IsVehicleExtraTurnedOn(veh, UTIL:LoadVehicleConfig(veh)?.stages.three[1]) then
										for _,k in pairs(UTIL:LoadVehicleConfig(veh)?.stages.one) do
									...
									if UTIL:LoadVehicleConfig(veh)?.stages ~= nil then
										for _,k in pairs(UTIL:LoadVehicleConfig(veh)?.stages.one) do
											SetVehicleExtra(veh, k, 0)
										end
										for _,k in pairs(UTIL:LoadVehicleConfig(veh)?.stages.two) do
			...
			if GetVehicleClass(veh) ~= 14 and GetVehicleClass(veh) ~= 15 and GetVehicleClass(veh) ~= 16 and GetVehicleClass(veh) ~= 21 then
				...
				if count_bcast_timer > delay_bcast_timer then
					...
					--- IS EMERG VEHICLE ---
					if GetVehicleClass(veh) == 18 then
						TriggerServerEvent('spark:TogDfltSrnMuted_s')
						TriggerServerEvent('spark:SetLxSirenState_s', state_lxsiren[veh])
						TriggerServerEvent('spark:SetPwrcallState_s', state_pwrcall[veh])
						TriggerServerEvent('spark:SetAirManuState_s', state_airmanu[veh])
					end
					--- IS ANY OTHER VEHICLE ---
					TriggerServerEvent('spark:TogIndicState_s', state_indic[veh])
				else
