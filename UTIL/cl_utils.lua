...
---------------------------------------------------
FILE: cl_utils.lua
PURPOSE: Utilities for siren assignments and tables and other common functions.
---------------------------------------------------
...
---------------------------------------------------------------------
--[[Get the config for the player's vehicle.]]
function UTIL:LoadVehicleConfig(veh)
    local vehicleHash = GetEntityModel(veh)
    if vehicleHash == 0 then return {} end

    for _, configName in ipairs(configured_vehicles) do
        if GetHashKey(configName) == vehicleHash then
            local configPath = string.format("CONFIGS/%s.lua", configName)
            local file = LoadResourceFile(GetCurrentResourceName(), configPath)
            
            if not file then
                print("Error: Could not load config file: " .. configPath)
                return {}
            end
